HA$PBExportHeader$w_misth_zpoikog_grid.srw
$PBExportComments$
forward
global type w_misth_zpoikog_grid from w_pbgrid
end type
end forward
...
string is_tablename = "misth_zpoikog"
end type
global w_misth_zpoikog_grid w_misth_zpoikog_grid
...
// kodoikog
	lstring = adw.object.kodoikog[row]
	if isnull(lstring) or lstring = "" then
		Messagebox(gs_app_name, trn(162))
		adw.setfocus()
		adw.setcolumn("kodoikog")
		return false
	end if
	
	// ¸ëåã÷ïò áí ï êùäéêüò õðÜñ÷åé
		ll_found = adw.find("kodoikog = '" + lstring + "'", 1, adw.rowcount())
		if ll_found = row then ll_found = adw.find("kodoikog = '" + lstring + "'", ll_found + 1, adw.rowcount())
...
// descoikog
	lstring = adw.object.descoikog[row]
	if isnull(lstring) or lstring = "" then
		Messagebox(gs_app_name, trn(175))
		adw.setfocus()
		adw.setcolumn("descoikog")
		return false
	end if
	
	// ¸ëåã÷ïò áí ç ðåñéãñáöÞ õðÜñ÷åé
		ll_found = adw.find("descoikog = '" + lstring + "'", 1, adw.rowcount())
		if ll_found = row then ll_found = adw.find("descoikog = '" + lstring + "'", ll_found + 1, adw.rowcount())
		if ll_found > 0 and ll_found <> row then
...
on w_misth_zpoikog_grid.create
call super::create
end on

on w_misth_zpoikog_grid.destroy
call super::destroy
...
type dw_main from w_pbgrid`dw_main within w_misth_zpoikog_grid
integer width = 1819
integer height = 1300
string dataobject = "dw_misth_zpoikog_list"
end type
