HA$PBExportHeader$wprn_report.srw
$PBExportComments$
forward
global type wprn_report from w_print_args_noform
end type
end forward
...
global wprn_report wprn_report
...
// ÃñÜöïõìå ôá óôïé÷åßá ôçò ìéóèïäïóßáò óôï dw
	date		ldt_datefinal
	string	ls_title
	select datefinal, title 
	into   :ldt_datefinal, :ls_title
	from   misth_final 
	where  kodfinal = :il_kodfinal and kodxrisi = :gs_kodxrisi;
	fn_sqlerror()
	
	dw.object.t_finaltitle.text = ls_title
	dw.object.t_kodfinal.text = string(il_kodfinal)	
	dw.object.t_datefinal.text = string(ldt_datefinal)
...
public subroutine of_retrieve ();dw.retrieve(is_kodreport, gs_kodxrisi)
end subroutine
...
ls_expresion = 'fn_getfromfinal(' + string(il_kodfinal) + ', misth_report_ypal_kodypal, ' + '~"' + asc_col.expr + '~")'
...
// t_title, t_finaltitle, t_subtitle
	li_x = integer(adw.object.t_title.x)
	li_width1 = ai_x - li_x
	adw.object.t_title.width = li_width1
	adw.object.t_finaltitle.width = li_width1
	adw.object.t_subtitle.width = li_width1
	
// t_kodfinal, t_datefinal
// t_kodfinal_t, t_datefinal_t
	li_width1 = integer(adw.object.t_kodfinal.width)
	li_width2 = integer(adw.object.t_kodfinal_t.width)
	adw.object.t_kodfinal.x = ai_x - li_width1
	adw.object.t_datefinal.x = ai_x - li_width1
	li_x = integer(adw.object.t_kodfinal.x)			// new x position
	adw.object.t_kodfinal_t.x = li_x - li_width2
	adw.object.t_datefinal_t.x = li_x - li_width2
	
end subroutine

on wprn_report.create
call super::create
end on

on wprn_report.destroy
call super::destroy
...
type dw from w_print_args_noform`dw within wprn_report
string dataobject = "sprn_report2"
end type
